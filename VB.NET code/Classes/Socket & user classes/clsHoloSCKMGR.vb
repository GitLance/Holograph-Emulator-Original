...
Public Class clsHoloSCKMGR
    ...
    Public Sub setupListener()
        Dim localHost As New IPEndPoint(IPAddress.Any, HoloRACK.sPort)
        ...
        Try '// Try setting up our SCKMGR on the port set in HoloRACK
            ...
            Console.WriteLine("[SCKMGR] Listening on port " & HoloRACK.sPort & ".")
        ...
        Catch ex As Exception '// Noes,  setting up failed! Report it and stop server
            Console.WriteLine("[SCKMGR] Error encountered while setting up SCKMGR on port " & HoloRACK.sPort & ", probably another application listens on this port already.")
    ...
    Private Sub connectionRequestCallback(ByVal ar As IAsyncResult)
        ...
        For s = 1 To HoloRACK.maxConnections
        ...
        If newClassID = 0 Then '// If the free socket returned was 0, then there were no sockets left! Change config.ini much?
            Console.WriteLine("[SCKMGR] Connection refused, the SCKMGR has ran out of it's " + HoloRACK.maxConnections & " free slots!")
            Return
        End If
